...
export class CypressConfigTransformer {
  ...
  private static propertiesToRemove: CypressConfigPropertyPaths[] = [];

  private static sourceFile: SourceFile;

  static removeProperties(
    existingConfigContent: string,
    toRemove: CypressConfigPropertyPaths[]
  ): string {
  ...
  private static buildMetadataFromConfig(
    ...
    propertiesToRemove: CypressConfigPropertyPaths[] = this.propertiesToRemove,
    parentPrefix = ''
  ): void {
    for (const property of config.properties) {
      ...
      if (
        propertiesToRemove.includes(propertyPath as CypressConfigPropertyPaths)
      ) {
        continue;
      }
